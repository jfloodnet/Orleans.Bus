...
namespace Orleans.Bus
{
    ...
    public abstract class StorageProvider<TState> : IStorageProvider
    {
        ...
        {
            return ReadStateAsync(grainReference.Id(), new GrainType(grainType), Get(grainState));
        }
        ...
        static TState Get(IGrainState grainState)
        {
            return ((IStateHolder<TState>)grainState).State;
        }
        ...
        /// <param name="state">Grain state to be read into.</param>
        /// <returns>
        /// Completion promise which return state for the specified grain.
        /// </returns>
        public abstract Task ReadStateAsync(string id, GrainType type, TState state);
